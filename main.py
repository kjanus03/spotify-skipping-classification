...
from data_preprocessing import preprocess_data, process_platform, process_skipped, process_shuffle, \
    undersample_preprocessed_data, oversample_preprocessed_data, remove_outliers
...
def main():
    ...
    streaming_history = load_data(sun_times, from_csv=False, file_paths=file_paths)
    ...
    streaming_history['skipped_last_time'] = streaming_history.groupby('spotify_track_uri')['skipped'].shift().fillna(
        False).astype(int)  # czy skipnieta przy poprzednim odsluchaniu
    ...
    create_report(streaming_history, 'Output/')
    # streaming_history = remove_outliers(streaming_history, numerical_features)
